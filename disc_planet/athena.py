...
class Athena3DSimulation(Simulation):
    """
    Simulation setup using Amelia Cordwell's 3D Athena++ spherical codes
...
class Athena2DSimulation(Simulation):
    """
    Class to for simulations computed using Amelia Cordwell's 2D Athena++
    diskplanet_alphabeta_2d.cpp problem file with hdf5 output.
    ...
    def process_1d_outputs(self, *args, include_vortensity=False, **kwargs):
        """ 
        Custom 1D output function for the case that the x1av variables were 
        output from the athena++ simulation 
        """
